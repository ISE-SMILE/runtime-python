...
def methodWithDecorator(code,decoratorName):

    pTree = parse(code)
    functions = filter(
        ##we only need to consider functions that have at least one decorator
        lambda x: len(x.decorator_list) > 0,
        ## we only need to consider functions
        filter(lambda x:isinstance(x,FunctionDef),
               pTree.body))
    name = None
    try:
        for funct in functions:
            if decoratorName in map(lambda x:x.id,funct.decorator_list):
                name =  funct.name
                break
    except:
        name = None
...
class PythonRunner(ActionRunner):
    ...
    def features(self):
        return json.dumps(self.featureFlags)
    ...
    def build(self, message):
        ...
        try:
            ...
            pauseFn = methodWithDecorator(code, "onPause")
            startFn = methodWithDecorator(code, "onStart")
            finishFn = methodWithDecorator(code, "onFinish")

            onRun = methodWithDecorator(code, "onRun")
            if onRun:
                self.mainFn = onRun

            if pauseFn:
                self.pauseFn = pauseFn
                self.featureFlags["onPauseHandlerOK"] = True

            if startFn:
                self.startFn = startFn
                self.featureFlags["onStartHandlerOK"] = True

            if finishFn:
                self.finishFn = finishFn
                self.featureFlags["onFinishHandlerOK"] = True

            logging.info("build action with %s"%self.features())
    ...
    def start(self, args, env):
        if self.featureFlags["onStartHandlerOK"]:
            result = self.exec_method(self.startFn,args, env)
    ...
    def pause(self, args, env):
        if self.featureFlags["onPauseHandlerOK"]:
            result = self.exec_method(self.pauseFn,args, env)
    ...
    def stop(self, args, env):
        if self.featureFlags["onFinishHandlerOK"]:
            result = self.exec_method(self.finishFn,args, env)
            if result and isinstance(result, dict):
                return (200, result)
    ...
    def run(self, args, env):
        result = self.exec_method(self.mainFn,args, env)
    ...
    def exec_method(self, method, args, env):
        result = None
        try:
            os.environ = env
            self.global_context['param'] = args

            exec('fun = %s(param)' % method, self.global_context)
            result = self.global_context['fun']
            logging.info("running %s with %s"%(method,result))
        except Exception:
